...
static void GetOSInformation(const FunctionCallbackInfo<Value>& args) {
  ...
  args.GetReturnValue().Set(Array::New(
      env->isolate(), osInformation, node::arraysize(osInformation)));
}
...
static void GetCPUInfo(const FunctionCallbackInfo<Value>& args) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    result.emplace_back(node::OneByteString(isolate, ci->model));
...
static void GetInterfaceAddresses(const FunctionCallbackInfo<Value>& args) {
  ...
  for (i = 0; i < count; i++) {
    ...
    result.emplace_back(node::OneByteString(isolate, ip));
    result.emplace_back(node::OneByteString(isolate, netmask));
    result.emplace_back(family);
    result.emplace_back(node::FIXED_ONE_BYTE_STRING(isolate, mac));
    result.emplace_back(interfaces[i].is_internal ? True(isolate)
                                                  : False(isolate));
...
static void GetUserInfo(const FunctionCallbackInfo<Value>& args) {
  ...
  if (args[0]->IsObject()) {
    ...
    if (!maybe_encoding.ToLocal(&encoding_opt)) return;

    encoding = ParseEncoding(env->isolate(), encoding_opt, node::UTF8);
  } else {
    encoding = node::UTF8;
  }
  ...
  auto free_passwd = node::OnScopeLeave([&]() { uv_os_free_passwd(&pwd); });
  ...
  MaybeLocal<Value> username =
      node::StringBytes::Encode(env->isolate(), pwd.username, encoding, &error);
  MaybeLocal<Value> homedir =
      node::StringBytes::Encode(env->isolate(), pwd.homedir, encoding, &error);
  ...
  if (pwd.shell == nullptr)
    ...
    shell =
        node::StringBytes::Encode(env->isolate(), pwd.shell, encoding, &error);
